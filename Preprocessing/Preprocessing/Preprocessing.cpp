...
std::vector<std::vector<int>> findBestMatch(std::vector<std::vector<std::vector<cv::Point2f>>> puzzle, int piece, int side, int num, bool flat[70][4]){
  ...
  if (flat[piece][side]){
    ...
    issue[0].push_back(piece);
    issue[0].push_back(side);
    return  issue;
  }
  for (int n = 0; n < puzzle.size(); n++) {
    for (int i = 0; i < puzzle[n].size(); i++) {
      if (flat[n][i]) {}
        ...
        float error = compareSide(base, puzzle[n][i], 1, 100);
        if (error < small[num]) {
          small.erase(small.begin() + 0);
          small.push_back(error);
          match.erase(match.begin() + 0);
          match[num - 1].push_back(n);
          match[num - 1].push_back(i);
        }
      }
    }
  }
...
int main() {
  ...
  */
  ...
  for (int n = 0; n < number_of_pieces; n++) {
    int count = 0;
    for (int k = 0; k < 4; k++){
      flat_piece[n][k] = isFlat(flat, n, k, 200);
      count += flat_piece[n][k]? 1 : 0;\
      std::cout << n << "\t " << count << "\r";
    }
    if (count == 2) {
      start = n;
  ...
  for (int offset = 0, state = 0; state != 1 && offset < 4; offset++) {
    if (flat_piece[start][(0+offset) % 4] && flat_piece[start][(1+offset) % 4])
  ...
  for (int y = 0; y < 7; y++){
    int x = 0;
    std::vector<std::vector<int>> match = findBestMatch(master_sorted_diviation,array[x][y][5], array[x][y][3], sample_size, flat_piece);
    if (0) {
      if (y == 9) { 
        for (int counter = 0, state = sample_size - 1; counter >= 0 && state != 0; counter--){
  ...
  for (int x = 0; x < 10; x++){
    int y = 0;
    std::vector<std::vector<int>> match = findBestMatch(master_sorted_diviation,array[x][y][5], array[x][y][2], sample_size, flat_piece);
    if (0) {
      if (x == 9) { 
        for (int counter, state = sample_size - 1; counter >= 0 && state != 0; counter--){
          if (flat_piece[match[counter][0]][(match[counter][1] + 1) % 4] && flat_piece[match[counter][0]][(match[counter][1] - 2) % 4]) {
            array[(x + 1) % 10][y + (x / 10)][5] = match[counter][0];
            array[(x + 1) % 10][y + (x / 10)][0] = (match[counter][1] - 1) % 4;
            array[(x + 1) % 10][y + (x / 10)][1] = (match[counter][1] + 0) % 4;
